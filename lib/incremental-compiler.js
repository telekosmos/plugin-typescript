...
export class IncrementalCompiler {
	constructor(fetch, resolve) {
		...
		this.loadedDefaultLib = this.resolve('typescript/bin/lib.d.ts')
			.then((res) => {
				this._servicesHost.setDefaultLibFilename(normalize(res));
	...
	load(filename) {
		...
		if (!this._files[filename]) {
			...
			this._files[filename].tsname = normalize(filename);

			/* file.loaded is a promise which returns the file
				when deps and text are populated and the file
				has been added to the host with any ambient overrides applied */
			this._files[filename].loaded = this.fetch(filename)
				...
				.then((depsMap) => {
					/* get a list of all the resolved dependencies for this file */
					this._files[filename].deps = Object.keys(depsMap).map((key) => depsMap[key]);

					/* trigger fetching its dependencies */
					this._files[filename].deps.forEach((res) => this.load(res));

					/* replace ambients with resolved filenames */
					var text = this.replaceAmbients(filename, this._files[filename].text, depsMap);
	 ...
	 */
	replaceAmbients(filename, text, depsMap) {
		var refsMap = undefined;
		var modsMap = undefined;

		Object.keys(depsMap).forEach((key) => {
			if (isAmbient(key)) {
				modsMap = modsMap || {};
				modsMap[key] = toModule(this._files[depsMap[key]].tsname);

				if (isTypescriptDeclaration(depsMap[key])) {
					refsMap = refsMap || {};
					refsMap[key] = this._files[depsMap[key]].tsname;
				}
			}
		});

		if (refsMap)
			text = this.mapReplace(text, refsMap);

		if (modsMap && !isTypescriptDeclaration(filename))
			text = this.replaceRequires(text, modsMap);
	...
	/**
	 * Replaces the ambient typescript requires with their resolved paths
	 *
	 ...
	 * @api private
	 */
	replaceRequires(text, map) {
		var therequires = requires(text);

		for (var i = 0; i < therequires.length; i ++) {
			var therequire = therequires[i];

			if (map[therequire.path]) {
				var quote = therequire.string.match(/"/) ? '"' : "'";
				var resolvedRequire = 'require(' + quote + map[therequire.path] + quote + ')';
				text = text.replace(therequire.string, resolvedRequire);
			}
		}
	...
	getDependencies(filename, text) {
		...
		var deps = info.referencedFiles.map((ref) => ref.filename);

		/* dts-bundle puts requires in declaration files, don't try and
			resolve them as they are ambient modules defined in the same file */
		if (!isTypescriptDeclaration(filename))
